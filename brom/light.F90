...
module fabm_niva_light
   ...
   type,extends(type_base_model),public :: type_niva_light
...
!
! !INPUT PARAMETERS:
      class (type_niva_light),intent(inout),target :: self
...
!BOC
      ...
      ! Note: FIS commented the silt component here and below 18/03/2022
!      call self%register_dependency(self%id_ESS, type_bulk_standard_variable(name='mass_concentration_of_silt'))
   ...
   subroutine get_light(self,_ARGUMENTS_VERTICAL_)
      class (type_niva_light),intent(in) :: self
      ...
      _VERTICAL_LOOP_BEGIN_
         ...
         xEPS = xEPS + self%EPS0X !! + self%EPSESSX*ESS w/o silt !!!
         xtnc = xEPS*dz
